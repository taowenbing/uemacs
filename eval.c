...
char *gtfun(char *fname)
{
	...
	case UFIND:
		result[0] = '%';
		/* strcpy(&result[1], getval(arg1));
		return strcpy(result, getval(result)); */
		getval(arg1, &result[1], sizeof(result) - 1);
		return getval(result, result, sizeof(result));
	case UFAND:
	...
	case UFUPPER:
		return mkupper(strcpy(result, arg1));
	case UFLOWER:
		return mklower(strcpy(result, arg1));
	case UFTRUTH:
